...
var sortArray = function(nums) {
    return shuffle(nums) && quickSort3Way(nums, 0, nums.length - 1)
};
...
function quickSort3Way(array, lo, hi) {
    ...
    const [lt, gt] = partition(array, lo, hi)
...
function partition(array, lo, hi) {
    ...
    while (i <= gt) {
        ...
        else if (array[i] > v) swap(array, gt--, i)
        else                   i++
    }
...
if (typeof test === 'boolean' && test) {
    ...
    let a
    expect(partition(a = [1,3,2], 0, 2), [0, 0]) && expect(a, [1,2,3]) //?. $
    expect(partition(a = [1,3,2], 1, 2), [2, 2]) && expect(a, [1,2,3]) //?. $
    expect(partition(a = [2,5,1,4,7], 0, 4), [1, 1]) && expect(a, [1,2,4,7,5]) //?. $
